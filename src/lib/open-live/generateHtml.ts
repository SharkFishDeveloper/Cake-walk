...
const generateHTML = (graph: Graph, start: string): string => `
  ...
  <head>
    ...
    <style>
      ...
      canvas {
        display: block;
        background-color: #222222; /* Dark canvas background */
  ...
  <body>
    ...
    <script>
      ...
      const drawNode = (x, y, name, import_name, childCount) => {
        ...
        ctx.fillStyle = "#ffffff"; /* White fill for nodes */
        ctx.fill();
        ctx.stroke();
        ctx.fillStyle = "green"; /* Green color for import name */
        ctx.font = \`\${8 * zoom}px Arial\`;
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText(import_name, x * zoom + offsetX, y * zoom + offsetY);

        ctx.fillStyle = "#ffffff"; /* White text for child count */
        ctx.font = \`\${4 * zoom}px Arial\`;
        ctx.textAlign = "right";
        ctx.fillText(\`Children: \${childCount}\`, x * zoom + offsetX - nodeRadius - 30, y * zoom + offsetY);

        ctx.fillStyle = "#ffffff"; /* White text for import name */
      ...
      const drawEdge = (x1, y1, x2, y2) => {
        ...
        ctx.strokeStyle = "#ffffff"; /* White edges */
        ctx.lineWidth = 2 * zoom;
        ctx.stroke();
      };

      // Recursive function to render the graph starting from the root node
      const renderGraph = (node, x, y) => {
        ...
        children.forEach(({ child, import_name }) => {
          ...
          // Only draw edges if they are visible
          if (visibleEdges[\`\${node}-\${child}\`] !== false) {
      ...
      canvas.addEventListener("click", (e) => {
        ...
        nodePositions.forEach(({ x, y, name }) => {
          ...
          if (distance <= nodeRadius * zoom) {
            // Log the node's name and the graph data structure
            console.log("Node clicked:", name);
            console.log("Graph data structure:", graph);
